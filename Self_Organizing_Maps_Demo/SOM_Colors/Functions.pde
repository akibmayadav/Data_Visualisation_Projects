float[][] kohonen_weights()
{
 for (int i = 0 ;i < total_number_of_nodes;i++)
 {
   ...
   nodes_array[i][0]=random(upper_node_weight[0]+1);
   nodes_array[i][1]=random(upper_node_weight[0]+1);
   nodes_array[i][2]=random(upper_node_weight[0]+1);
 }
...
void draw_nodes()
{
  ...
  for ( int i = 0 ; i < nodes_horizontal ;i++)
  {
    ...
    {
     fill(nodes_array[i+j*nodes_horizontal][0],nodes_array[i+j*nodes_horizontal][1],nodes_array[i+j*nodes_horizontal][2],100);
...
void draw_input_vectors()
{
  ...
  for (int i = 0 ; i <input_vectors.length ;i++)
  {
    fill(input_vectors[i][0],input_vectors[i][1],input_vectors[i][2],100);
...
void train_SOM()
{
  ...
  if ( iterationCounter<max_iterations)
  {
    float y_start = height_margin+ radius;
    float x_start = width_margin + radius;
    neighbour_distance = 82+((height-2*height_margin)/4*(1-(iterationCounter/max_iterations)));
    ...
    for ( int i = 0 ; i < access_input_vectors.length; i++)
    {
      ...
      fill(255,255,255,50);
      //ellipse(bmuX,bmuY,neighbour_distance,neighbour_distance);
    ...
    for (int node=0; node<nodes_array.length; node++){
      ...
      for (int i=0; i<number_of_node_weights; i++){
        ...
        if(node%nodes_horizontal != 0){
          adjacentWeights += nodes_array[node-1][i]; //if you're along the left side of the screen don't check node to left
          numWeightsChecked++;
        }
        if(node%nodes_horizontal != nodes_horizontal-1){
          adjacentWeights += nodes_array[node+1][i];//if you're along the right don't check to the right
          numWeightsChecked++;
        }
        if(node>=nodes_horizontal){
          adjacentWeights += nodes_array[node-nodes_horizontal][i];//if you're along the top don't check above you
          numWeightsChecked++;
        }
        if(node<(nodes_array.length-nodes_horizontal)){
          adjacentWeights += nodes_array[node+nodes_horizontal][i];//if you're along the bottem don't check below yourself
          numWeightsChecked++;
        }
