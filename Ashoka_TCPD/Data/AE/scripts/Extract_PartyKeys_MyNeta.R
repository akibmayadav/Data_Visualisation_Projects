...
infile<-unique(subset(infile,infile$Year>=2005,select=c("Year","Party1")))

myneta<-unique(subset(myneta,select=c("Year","Party_Code")))

#First find those constituency names where there is exact match (modulo capital letters conversion)
myneta<-setnames(myneta,old=c("Year","Party_Code"),new=c("Year","Party1"))

##convert ac_name to capital letters in infile and myneta frames
myneta$Party1<-canonicalizePartyNames(myneta$Party1)
myneta$Year<-trimws(myneta$Year)
myneta$src<-"mn"

infile$Party1<-canonicalizePartyNames(infile$Party1)
...
common<-inner_join(myneta,infile,by=c("Year","Party1"))
#removing two unnecessary columns added because of join
common$src.x<-NULL
common$src.y<-NULL
#Get the rest of the names where there is no match, we will need manual intervention in these cases
rest<-union(anti_join(myneta,infile,by=c("Year","Party1")),
            anti_join(infile,myneta,by=c("Year","Party1")))


#store common in a common file in the format year, ec_Party_Name, mn_Party_Name, for this drop src and create anotehr column after copying Party1
                                        #and rename it to mn_Party_Name. Rename the earlier Party1 column to ec_Party_Name
common$src<-NULL
common<-unique(common)
common$mn_Party_Name<-common$Party1
common$ec_Party_Name<-common$Party1
common$Party1<-NULL
write.csv(common,paste0(args[1],"/primary/myneta/common_partyname_keys.csv"),row.names=FALSE)
...
rest<-arrange(rest,Year,Party1)

write.csv(rest,paste0(args[1],"/primary/myneta/manual_partyname_keys.csv"),row.names=FALSE)
