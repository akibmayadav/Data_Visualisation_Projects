...
def csvtorecsv(inputfile):
	...
	f = open( inp, 'rU' )  
	f_out = open( oup , 'w' )    
	reader = csv.DictReader( f, fieldnames = ( "war_name" ,"start_year" ,"end_year" ))
	writer = csv.DictWriter( f_out, fieldnames = ("war_name" ,"start_year" ,"end_year"))
	count = 0
	for row in reader:
		count = count +1
		if (count%1 == 0) :
...
def recsvtojson(inputfile):
	oup = '/Users/akibmayadav/Documents/Personal_Webapge_Development/Processing_Code_Porting/Code_Of_Red/Data_Parsing_Into_JSON/Processed_Csv/1_' + inputfile ;
	f_out2 = open( oup , 'rU' )
	reader2 = csv.DictReader( f_out2, fieldnames = ( "war_name" ,"start_year" ,"end_year"))
	out_name = inputfile[:-4];
	out = "var WorldWide_War"+"= [\n\t" + ",\n\t".join([json.dumps(row) for row in reader2]) + "\n]"
...
csvtorecsv("WorldWide_War.csv")
recsvtojson("WorldWide_War.csv")
